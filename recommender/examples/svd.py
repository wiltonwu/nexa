...
path = parent_path + '/stocks/stock_users.dat'
...
# stocks_example = np.array(model.valid['item'])[0:10]
# actual_ratings = np.array(model.valid['rating'])[0:10]
# print(user_example)
# print(stocks_example)

fileHandle = open (path,"r" )
line_list = fileHandle.readlines()
fileHandle.close()
last_line = line_list[-1]
last_user = int(last_line.split("::")[0])
user_example = np.array([1] * 120)[0:100]
stocks_example = np.array(list(range(1, 121)))[0:100]

print(user_example)
print(stocks_example)

predicted_ratings = model.prediction(user_example, stocks_example)
...
print(max_predicted_rating)
itemindex = np.where(predicted_ratings==max_predicted_rating)
print(itemindex)
